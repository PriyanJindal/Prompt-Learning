...
generate a revised version of the original prompt that would definitely improve results with respect to the evaluation labels. 
...
FINAL INSTRUCTIONS
Iterate on the original prompt (above) with a new prompt that will improve the results, based on the examples and feedback above. Make sure to meticulously take all feedback into account. 
Make sure to keep good aspects of the original prompt, and only change the parts that are explicitly mentioned in the feedback.
If an example has good feedback, ensure that the new prompt still includes the good aspects of the original prompt.
