...
class TiktokenSplitter:
    ...
    def __init__(self, model: str = "gpt-4o"):
        ...
        """
        self.model = self.tiktoken_model_input(model)
        self.tiktoken_encoder = tiktoken.encoding_for_model(self.model)
        print(f"🔧 Initialized TiktokenSplitter with model: {self.model}")

    def tiktoken_model_input(self, model: str) -> str:
      # edge case for gpt-4.1
      if model.startswith("gpt-4.1"):
          return "gpt-4o"
  
      # --- ordinary cases ------------------------------------------------------
      if model in SUPPORTED_MODELS:                     # exact hit
          return model
  
      # check prefix matches, preferring longer keys first
      for key in sorted(SUPPORTED_MODELS, key=len, reverse=True):
    ...
    def create_batches(self, 
                      ...
                      show_progress: bool = True) -> List[Tuple[int, int]]:
        """
    ...
    def get_batch_dataframes(self, 
                           df: pd.DataFrame, 
                           columns: List[str], 
                           context_size_tokens: int) -> List[pd.DataFrame]:
        """
